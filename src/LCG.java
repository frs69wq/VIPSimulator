...
public class LCG {
	...
	public static void crInput(Host issuerHost, String logicalFileName, long logicalFileSize, String SEName, String LFCName) {
		Msg.info("Ask '"+ LFCName + "' to register input file '" + logicalFileName + "' stored on SE '" + SEName + "'");
		Message task = new Message(Message.Type.CR_INPUT, issuerHost, logicalFileName, logicalFileSize, SEName);
		task.emit(LFCName);
		Msg.debug("lcg-cr-input of '" + logicalFileName +"' on LFC '" + LFCName +"' completed");
	}
	
	public static void cr(Host issuerHost, String SEName, String localFileName, long localFileSize, String logicalFileName, String LFCName) {
		Msg.debug("lcg-cr " + logicalFileName + " from " + localFileName + "using lfc " + LFCName);

		//TODO this request should lead to a communication whose cost is related to the size of the local file
		...
		Msg.info("Ask '"+ LFCName + "' to register file '" + logicalFileName + "' stored on SE '" + SEName + "'");
		Message askToRegisterFile = new Message(Message.Type.REGISTER_FILE, issuerHost, logicalFileName, localFileSize, SEName);
	...
	public static void cp(Host issuerHost, String logicalFileName, String localFileName, String LFCName){
		String SEName = null;
		long logicalFileSize = 0;
		
		Msg.info("lcg-cp '" + logicalFileName + "' to '" + localFileName + "' using LFC '" + LFCName + "'");
		
		//get SE name from LFC
		Message askFileInfo = new Message(Message.Type.ASK_FILE_INFO, issuerHost, logicalFileName);
		askFileInfo.emit(LFCName);
		Msg.info("Asked SE name to LFC " + LFCName + " for file " + logicalFileName);
		...
		Msg.info("Downloading file " + logicalFileName + " from SE " + SEName + " using LFC " + LFCName);
		Message downloadRequest = new Message(Message.Type.DOWNLOAD_REQUEST, issuerHost, logicalFileName, logicalFileSize);
		
		downloadRequest.emit(SEName);
		Msg.info("Sent download request to SE '" + SEName + "' for file '" + logicalFileName +"' of size " + logicalFileSize);
